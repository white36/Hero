...
static void vision_task_feedback_update(vision_control_t* update)
{
    ...
    // 修正当前弹速
    calc_current_bullet_speed(update,BULLET_42,SHOOTER_42);
    // 存放偏差时间
...
static void vision_data_process(vision_control_t* vision_data)
{
    ...
    if (vision_data->vision_target_appear_state == TARGET_APPEAR)
    {
        ...
        // 计算机器人pitch轴与yaw轴角度
        vision_data->gimbal_vision_control.gimbal_pitch = calc_target_position_pitch_angle(&vision_data->solve_trajectory, 
                                                                                           sqrt(pow(vision_data->robot_gimbal_aim_vector.x, 2) + pow(vision_data->robot_gimbal_aim_vector.y, 2)) - vision_data->solve_trajectory.distance_static, 
                                                                                           vision_data->robot_gimbal_aim_vector.z + vision_data->solve_trajectory.z_static);
...
//        {
          ...
          //  {
                //修正弹速 -- 修正精度到整数
                calc_cur_bullet_speed->bullet_speed =16.0f; //(int16_t)calc_cur_bullet_speed->shoot_data_point->bullet_speed;
         //   }
//        }
//    }
}
...
/**
 * @brief 计算弹道落点 -- 完全空气阻力模型 该模型适用于大仰角击打的击打（暂无应用）
 * 
...
//    }
    ...
    {
        pitch = atan2(aim_z, x);
        solve_trajectory->flight_time = (float)((exp(solve_trajectory->k1 * x) - 1) / (solve_trajectory->k1 * solve_trajectory->current_bullet_speed * cos(pitch)));
        bullet_drop_z = (((quality_42 * GRAVITY) / (solve_trajectory->k1 * solve_trajectory->current_bullet_speed * cos(pitch))) + 
                        (solve_trajectory->current_bullet_speed * sin(pitch) / solve_trajectory->current_bullet_speed * cos(pitch))) * x - 
                        ((quality_42 * quality_42 * GRAVITY) / (solve_trajectory->k1 * solve_trajectory->k1)) * log((quality_42 * solve_trajectory->current_bullet_speed * cos(pitch)) / 
                        (quality_42 * solve_trajectory->current_bullet_speed * cos(pitch) - solve_trajectory->k1 * x));
        calc_and_actual_error = z - bullet_drop_z;
        aim_z = aim_z + calc_and_actual_error;
        if (fabs(calc_and_actual_error) < PRECISION)
        {
            break;
        }
