...
void gimbal_behaviour_mode_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    //根据云台行为状态机设置电机状态机
    if (gimbal_behaviour == GIMBAL_ZERO_FORCE)//掉电
    {
    ...
    else if (gimbal_behaviour == GIMBAL_ABSOLUTE_ANGLE)//绝对角度
    {
    ...
    else if (gimbal_behaviour == GIMBAL_MOTIONLESS)
    {
		...
		else if (gimbal_behaviour == GIMBAL_ABSOLUTE_SPIN)//启用
    {
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_GYRO;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_GYRO;
    }
...
//            !switch_is_down(gimbal_mode_set->gimbal_rc_ctrl->rc.s[0]) && !toe_is_error(DBUS_TOE))
//        {
//            return;
//        }
//        else
...
//    if(robot_state.power_management_gimbal_output == 0)
//		{
//        gimbal_behaviour = GIMBAL_ZERO_FORCE;
//		}
...
//			  //int_abs为绝对值函数
//        if (int_abs(gimbal_mode_set->gimbal_rc_ctrl->rc.ch[0]) < GIMBAL_MOTIONLESS_RC_DEADLINE && 
//					  int_abs(gimbal_mode_set->gimbal_rc_ctrl->rc.ch[1]) < GIMBAL_MOTIONLESS_RC_DEADLINE && 
//				    int_abs(gimbal_mode_set->gimbal_rc_ctrl->rc.ch[2]) < GIMBAL_MOTIONLESS_RC_DEADLINE && 
//				    int_abs(gimbal_mode_set->gimbal_rc_ctrl->rc.ch[3]) < GIMBAL_MOTIONLESS_RC_DEADLINE && 
//				    int_abs(gimbal_mode_set->gimbal_rc_ctrl->mouse.x) < GIMBAL_MOTIONLESS_RC_DEADLINE && 
//				    int_abs(gimbal_mode_set->gimbal_rc_ctrl->mouse.y) < GIMBAL_MOTIONLESS_RC_DEADLINE && 
//				    gimbal_mode_set->gimbal_rc_ctrl->key.v == 0 && 
//				    gimbal_mode_set->gimbal_rc_ctrl->mouse.press_l == 0 && 
//				    gimbal_mode_set->gimbal_rc_ctrl->mouse.press_r == 0)      
//        {
//					
...
static void gimbal_absolute_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    static int16_t yaw_channel = 0, pitch_channel = 0;
		...
		if(!last_key_F && gimbal_control_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_F)
		{
				limit=!limit;
		}
    ...
    rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch[PITCH_CHANNEL], pitch_channel, RC_DEADBAND);
		//临时
//		rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch[PITCH_CHANNEL], yaw_channel, RC_DEADBAND);
//		rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch[YAW_CHANNEL], pitch_channel, RC_DEADBAND);
		if(limit == 1){	
    ...
    *pitch = pitch_channel * PITCH_RC_SEN + gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN*0.2f;
		...
		*yaw = yaw_channel * YAW_RC_SEN - gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN*0.8f -vision_rx->ang_z;
    *pitch = pitch_channel * PITCH_RC_SEN + gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN*0.8f;
			//临时
//    *pitch = pitch_channel * PITCH_RC_SEN *0.22f + gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN*0.8f;
		}
		last_key_F = gimbal_control_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_F;
}
...
//     // 赋值增量
// 	if(yaw_set_angle&&pitch_set_angle!=0)
// 		{
