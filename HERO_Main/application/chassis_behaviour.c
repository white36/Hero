...
{   
    ...
    //遥控器设置模式
		if(!switch_is_down(chassis_move_mode->chassis_RC->rc.s[1]))//左
		{
...
else if(switch_is_down(chassis_move_mode->chassis_RC->rc.s[1]))//干什么的
		{
	            if(mode == 1)
				{
						chassis_behaviour_mode = CHASSIS_NO_FOLLOW_YAW;//比赛用 底盘不跟随，可能用于射击
				}
				else if(mode == 2)
				{
...
//    if(robot_state.power_management_chassis_output == 0)
//		{
//        chassis_behaviour_mode = CHASSIS_ZERO_FORCE;
//		}
...
static void chassis_bpin_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
		switch (chassis_move.chassis_power_MAX)
		{
				case 55:
				{
						*vx_set*=0.8f;
